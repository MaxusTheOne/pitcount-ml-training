...
def npy_to_fmap(
    npy_path_or_array: Path | str | np.ndarray,
    size: tuple[int,int] = None
) -> np.ndarray:
    """
    Load a .npy microscopy image or accept a numpy array and return its raw VGG conv2_2 feature map.

    Args:
      npy_path_or_array: Path to your .npy file or a numpy array.
      size: (height, width) to resize for VGG before extracting features.

    Returns:
      fmap: numpy array of shape (H, W, 128), where H/W are the original image dims.
    """
    # Determine VGG input size
    vgg_input_size = size or (256, 256)

    # --- 1. Load or receive array and squeeze to 2D/3D ---
    if isinstance(npy_path_or_array, np.ndarray):
        arr = np.squeeze(npy_path_or_array)
    else:
        arr = np.load(str(npy_path_or_array))
        arr = np.squeeze(arr)
    ...
    if arr.ndim == 2:
        ...
        raise ValueError(f"Unexpected arr.ndim={arr.ndim} for {npy_path_or_array}")
...
def czi_to_npy(
    ...
    """
    Load a .czi image and returns a np.ndarray.

    Args:
      czi_path: Path to your .czi file.

    Returns:
      npy: numpy array. For displaying the image.
    """
    npy_image = czifile.imread(str(czi_path))
    return npy_image
